...
void main(void){
    vec2 screenSpaceCoords = (clipSpace.xy / clipSpace.w) / 2.0 + 0.5;
    ...
    vec2 distortedTexCoords = texture(dudvMap, vec2(textureCoords.x + moveFactorOffset, textureCoords.y)).rg * 0.1;
    distortedTexCoords = textureCoords + vec2(distortedTexCoords.x, distortedTexCoords.y + moveFactorOffset);
